...
<!-- claude-common:begin -->
<!-- Managed by claude-common v1.1.0 via scripts/sync-consumers.sh — edit upstream, not here. -->
@AGENT-DIRECTIVE.md

**Shared agent toolkit (claude-common v1.1.0).** Before building tooling, scripts, agents or hooks
...
```cron
0 9 * * 1 $HOME/repos/claude-common/scripts/cron-discover.sh >> $HOME/repos/claude-common/state/discover.log 2>&1
30 9 * * 1 AUTO_PUSH=1 $HOME/repos/claude-common/scripts/sync-consumers.sh >> $HOME/repos/claude-common/state/sync.log 2>&1
```
...
carrying `managed-by: claude-common`; `.claude/hooks/common/*.sh`; `.claude/skills/<name>/*` (SKILL.md
carries the marker, sibling files are plain copies); a jq-merged `.claude/settings.json`
(repo keys win); a marker block in `CLAUDE.md`; `.claude/common.lock`. The push uses `--no-verify`
(consumer pre-push hooks are dev-machine preflights; the PR is the gate). Repo-local files are never
